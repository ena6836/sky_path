...
def main():
    ...
    next_initial_pos = np.array(path_nodes[0][0])
...
def split_path(path_nodes, seg_num):
    num_edge = len(path_nodes)
    seg_len = num_edge // seg_num
...
def find_fin_node(seg_path):
    ...
    for i in range(seg_num):
        fin_nodes[i] = seg_path[i][len(seg_path[i])]
...
def make_real_skyline(node):
    sigma_azimuth = 1   #方位角の推定誤差
    sigma_photo = 0.001     #撮影時に生じるノイズ
    sigma_pixel = 0.001     #解像度による誤差
...
def skyline_with_error_azi(sigma, x_center, y_center):
    ...
    while azi < 360:
        while k <= max_k:
            x_on_azi_line =  round(x_center + k * delta_p * math.cos((azi + error) / 360 * 2 * math.pi) / grid_size)
            y_on_azi_line =  round(y_center + k * delta_p * math.sin((azi + error) / 360 * 2 * math.pi) / grid_size)
            curv = (radius_star - math.sqrt(radius_star ** 2 - (k * delta_p) ** 2) ) / grid_size
            height_interest = (dem[y_on_azi_line][x_on_azi_line] - curv) / grid_size
            height_center = (dem[y_center][x_center] + height_cam) / grid_size
            elevation_interest = math.atan((height_interest - height_center) / (k * delta_p / grid_size))
